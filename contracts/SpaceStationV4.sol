 ...
 * @author Orbit
 */
contract OrbitSpaceStationV4 is
    ...
    ExponentialNoError
{
    ...
    function _resetContributorSpeeds(
        ...
        OToken[] memory tokens
    ) public {
        require(adminOrInitializing(), "only admin can reset comp speeds");

        // reset the OToken market states
        for (uint i = 0; i < tokens.length; i++) {
            address oTokenAddy = address(tokens[i]);
            CompMarketState storage supplyState = compSupplyState[oTokenAddy];
            CompMarketState storage borrowState = compBorrowState[oTokenAddy];
            uint32 blockNumber = safe32(
                getBlockNumber(),
                "block number exceeds 32 bits"
            );
            ...
            compSupplySpeeds[oTokenAddy] = 0;
            compBorrowSpeeds[oTokenAddy] = 0;
            ...
            for (uint j = 0; j < contributors.length; j++) {
                ...
                delete compSupplierIndex[oTokenAddy][contributors[j]];
                delete compBorrowerIndex[oTokenAddy][contributors[j]];
