    ...
    TokenErrorReporter
{
    ...
    function _blastConfigureYield() external {
        if (msg.sender != admin) {
    ...
    function _blastClaimYield(address target) external {
        ...
        BlastContract.claimMaxGas(address(this), target);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _blastClaimYieldAll(address recipient) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            revert BlastYieldOwnerCheck();
        }
        ...
        uint256 usdbAmount = USDBBlast.getClaimableAmount(address(this));
        if (usdbAmount > 0) {
            USDBBlast.claim(recipient, usdbAmount);
        }

        uint256 wethAmount = WETHBlast.getClaimableAmount(address(this));
        if (wethAmount > 0) {
            WETHBlast.claim(recipient, wethAmount);
        }
